...
class ACL
{
    ...
    public static function checkShowReg($reg, $showerror = true) {
        ...
        if ($showerror == true) {
            System::setErrorMsg("Нет права просмотра  журнала  " . self::$_metasdesc[$reg]);
            App::RedirectError();
        }
        return false;
    }
    ...
    public static function checkShowDoc($doc, $inreg = false, $showerror = true,$user_id=0) {
        ...
        if ($user->onlymy == 1 && $doc->document_id > 0) {
            ...
            if ($user->user_id != $doc->user_id) {
                if ($showerror == true) {
                    System::setErrorMsg("Нет права просмотра документа  " . self::$_metasdesc[$doc->meta_name]);
                }
                if ($inreg == false) {
                    App::RedirectError();
                }
    ...
    public static function checkChangeStateDoc($doc, $inreg = true, $showerror = true,$user_id=0) {
        ...
        if ($showerror == true) {
            System::setErrorMsg("Нет права смены статуса документа " . self::$_metasdesc[$doc->meta_name]);
            if ($inreg == false) {
                App::RedirectError();
            }
        }
